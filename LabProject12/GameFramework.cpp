...
void CGameFramework::ProcessInput()
{
	...
	if (::GetCapture() == m_hWnd) {
		...
		cxDelta = (float)(ptCursorPos.x - m_ptOldCursorPos.x) / 3.f;
		cyDelta = (float)(ptCursorPos.y - m_ptOldCursorPos.y) / 3.f;
		::SetCursorPos(m_ptOldCursorPos.x, m_ptOldCursorPos.y);
	}

	if ((dwDirection != 0) || (cxDelta != 0.f) || (cyDelta != 0.f)) {
		if (cxDelta || cyDelta) {
			if (pKeyBuffer[VK_RBUTTON] & 0xF0) {
				m_pPlayer->Rotate(cyDelta, 0.f, -cxDelta);
			}
			else {
...
void CGameFramework::Render()
{
	...
	m_pd3dCommandList->ClearDepthStencilView(d3dDSVDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
#endif
	if (m_pPlayer) {
