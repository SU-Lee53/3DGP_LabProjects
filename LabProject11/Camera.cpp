...
CCamera::CCamera()
{
	...
	m_fRoll = 0.f;
	m_fYaw = 0.f;
	m_xmf3Offset = XMFLOAT3(0.f, 0.f, 0.f);
	m_fTimeLag = 0.f;
	m_xmf3LookAtWorld = XMFLOAT3(0.f, 0.f, 0.f);
	m_nMode = 0x00;
	m_pPlayer = NULL;
}
...
CCamera::CCamera(CCamera* pCamera)
{
	if (pCamera) {
		*this = *pCamera;
		...
		m_d3dScissorRect = { 0,0,FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT };
		m_xmf3Position = XMFLOAT3(0.f, 0.f, 0.f);
		m_xmf3Right = XMFLOAT3(0.f, 0.f, 0.f);
		m_xmf3Look = XMFLOAT3(0.f, 0.f, 0.f);
		m_xmf3Up = XMFLOAT3(0.f, 0.f, 0.f);
		m_fPitch = 0.f;
		m_fRoll = 0.f;
		m_fYaw = 0.f;
		m_xmf3Offset = XMFLOAT3(0.f, 0.f, 0.f);
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera)
{
	...
	if (pCamera) {
		// Before change to first person camera, If camera was SPACESHIP mode, Set camera's Up vector as y-axis
		// This means make camera to stand upright
...
void CFirstPersonCamera::Rotate(float fPitch, float fYaw, float fRoll)
{
	...
	if (fRoll != 0.f) {
		XMFLOAT3 xmf3Look = m_pPlayer->GetLookVector();
		XMMATRIX xmmtxRotate = XMMatrixRotationAxis(XMLoadFloat3(&xmf3Look), XMConvertToRadians(fYaw));
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer) {
		...
		// Camera Lag
		// if m_fTimeLag > 1 : fTimeScale value will smaller and actual rotate will be small
		// if m_fTimeLag != 0 : multiplied fTimeElapsed to make camera follows player after some delay(1.0f / m_fTimeLag)
