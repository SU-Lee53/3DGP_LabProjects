...
void CScene::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice)
{
	...
	for (int i = 0; i < m_nDemoShaders; ++i)
	{
		ID3DBlob* pd3dVertexShaderBlob = NULL;
		ID3DBlob* pd3dPixelShaderBlob = NULL;

		std::wstring wstrShaderName = L"05-01-";
		wstrShaderName = i >= 10 ? wstrShaderName + std::to_wstring(i) + L".hlsl" : wstrShaderName + L"0" + std::to_wstring(i) + L".hlsl";
		D3DCompileFromFile(wstrShaderName.c_str(), NULL, NULL, "VSMain", "vs_5_1", nCompilerFlags, 0, &pd3dVertexShaderBlob, NULL);
		D3DCompileFromFile(wstrShaderName.c_str(), NULL, NULL, "PSMain", "ps_5_1", nCompilerFlags, 0, &pd3dPixelShaderBlob, NULL);
		...
		pd3dDevice->CreateGraphicsPipelineState(&d3dPipelineStateDesc, IID_PPV_ARGS(&m_pd3dPipelineStates[i]));
...
void CScene::ReleaseObjects()
{
	...
	for (int i = 0; i < m_nDemoShaders; ++i) {
		if (m_pd3dPipelineStates[i])
			m_pd3dPipelineStates[i]->Release();
	}
}
...
void CScene::PrepareRender(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	// Set Pipeline State
	pd3dCommandList->SetPipelineState(m_pd3dPipelineStates[m_nDemoShaderIndex]);
