...
{
	// if bUpdateVelocity == TRUE, Update velocity vector. don't move player.
	if (bUpdateVelocity)
	{
		// Update velocity vector to xmf3Shift
		XMStoreFloat3(&m_xmf3Velocity, XMVectorAdd(XMLoadFloat3(&m_xmf3Velocity), XMLoadFloat3(&xmf3Shift)));
	}
	else
	{
		// Move player amount of xmf3Shift vectors from current position
		XMStoreFloat3(&m_xmf3Position, XMVectorAdd(XMLoadFloat3(&m_xmf3Position), XMLoadFloat3(&xmf3Shift)));

		// Since player's position is changed, Update camera too.
...
void CPlayer::Rotate(float fPitch, float fYaw, float fRoll)
{
	...
	XMVECTOR xmvRight = XMVector3Normalize(XMVector3Normalize(XMVector3Cross(xmvUp, xmvLook)));
...
void CPlayer::LookAt(const XMFLOAT3& xmf3LookAt, XMFLOAT3& xmf3Up)
{
...
// This function invokes in every each frame.
// Applies gravity and friction to velocity vector and move player.
void CPlayer::Update(float fTimeElapsed)
{
	...
	// if player's velecity vector needs to decelerates because of friction, creates decelerate vector from here.
	// decelerate vector's direction is opposite of speed vector.
...
{
	// Computes world matrix from player's position and direction vectors.
	OnUpdateTransform();
...
void CAirplanePlayer::OnUpdateTransform()
{
	...
	XMStoreFloat4x4(&m_xmf4x4World, XMMatrixMultiply(XMMatrixRotationRollPitchYaw(XMConvertToRadians(90.0f), 0.f, 0.f), XMLoadFloat4x4(&m_xmf4x4World)));
}