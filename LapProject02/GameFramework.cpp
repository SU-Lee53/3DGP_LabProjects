...
void CGameFramework::CreateSwapChain()
{
	...
	// Create SwapChain
	HRESULT hr = m_pdxgiFactory->CreateSwapChainForHwnd(m_pd3dCommandQueue, m_hWnd, &dxgiSwapChainDesc, &dxgiSwapChainFullScreenDesc, NULL, (IDXGISwapChain1**)&m_pdxgiSwapChain);
	if (FAILED(hr)) __debugbreak();

	// Disable alt+enter(fullscreen)
	m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);

	// Store current backbuffer index
